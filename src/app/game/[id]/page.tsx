...
const getData = async (id: string) => {
  try {
    ...
    throw new Error(`Não foi possível buscar pelo id ${id} - Error: ${error}`);
  }
};
...
const getGamesSorted = async () => {
  try {
    ...
    throw new Error(`Não foi possível buscar pelo jogo - Error: ${error}`);
  }
};

// Página do jogo
export default async function GameDetail({ params }: PropsParams) {
  const { id } = params;
